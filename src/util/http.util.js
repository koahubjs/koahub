        ...
        if (lodash.includes(methods, action)) {
            ...
            try {
                ...
                if (lodash.includes(methods, '_initialize')) {
                    parseResult(await getPromiseFunction(_ctrl, '_initialize')(...args));
                }
                // 控制器初始化不响应404，中断执行
                if (ctx.status != 404) {
                    return;
                ...
                if (lodash.includes(methods, '_before')) {
                    parseResult(await getPromiseFunction(_ctrl, '_before')(...args));
                }
                // 控制器前置不响应404，中断执行
                if (ctx.status != 404) {
                    return;
                ...
                if (lodash.includes(methods, `_before_${action}`)) {
                    parseResult(await getPromiseFunction(_ctrl, `_before_${action}`)(...args));
                }
                // 方法前置不响应404，中断执行
                if (ctx.status != 404) {
                    return;
                ...
                parseResult(await getPromiseFunction(_ctrl, action)(...args));
                // 不响应404，中断执行
                if (ctx.status != 404) {
                    return;
                }
                ...
                if (lodash.includes(methods, `_after_${action}`)) {
                    parseResult(await getPromiseFunction(_ctrl, `_after_${action}`)(...args));
                }
                // 方法后置不响应404，中断执行
                if (ctx.status != 404) {
                    return;
                ...
                if (lodash.includes(methods, '_after')) {
                    parseResult(await getPromiseFunction(_ctrl, '_after')(...args));
                }
                // 控制器后置不响应404，中断执行
                if (ctx.status != 404) {
                    return;
